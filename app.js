...
// Initialize browser state containers safely
...
const API = {
    ...
    addProductReview: (productId, userName, rating, comment) => {
        let db = getDB();
        let p = db.products.find(prod => prod.id === productId);
        if (p) {
            if (!p.reviews) p.reviews = [];
            p.reviews.push({ user: userName, rating: parseInt(rating), comment });
...
window.handleSignup = function(e) {
    ...
    if(res.status === 201) {
        alert('Account generated successfully! Please log in.');
        navigate('login');
    } else {
...
window.cancelUserOrder = function(orderId) {
    if(confirm("Confirm cancellation for order: " + orderId)) {
...
function renderProfile() {
    ...
    div.innerHTML = `
        ...
        <div class="bg-white p-6 rounded-lg shadow-md md:col-span-2">
            ...
            ${orders.length === 0 ? '<p class="text-gray-500 text-sm">No transaction records found.</p>' : `
                <div class="space-y-4">
                    ${orders.map(o => `
...
window.handleReviewSubmit = function(e, prodId) {
    ...
    alert("Review ledger updated successfully.");
    render();
}
...
function renderHome() {
    ...
    if(filteredProds.length === 0) {
        mainGrid.innerHTML = `<div class="bg-white p-12 rounded-lg text-center text-gray-500 shadow-sm font-medium">No catalog records match parameters.</div>`;
    } else {
...
function renderProductDetail() {
    ...
    if(!p) { div.innerText = "Product item missing."; return div; }
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="flex flex-col justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600 leading-relaxed border-t pt-4">Standard ZigZag premium tier distribution object.</p>
        ...
        <div class="mt-12 border-t pt-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="space-y-4 lg:col-span-2">
                    ${!p.reviews || p.reviews.length === 0 ? '<p class="text-gray-500 text-sm">No customer reviews submitted yet.</p>' : p.reviews.map(r => `
...
window.quickAddToCart = function(productId) {
    ...
    alert("Shopping cart manifest state tracking variable modified.");
    render();
}
    ...
    div.innerHTML = `
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                ${activeItems.length === 0 ? '<p class="text-gray-500 text-sm">Active cart manifest empty.</p>' : activeItems.map(item => {
...
window.executePlaceOrder = function(total) {
    ...
    alert(`Contacting external payment gateway engine.\nNetwork selected: ${method}\nTotal processed: ₹${total}`);
    const order = API.placeOrder(AppState.currentUser.id, AppState.checkoutCart, total, method);
    alert("Transaction verified. Order generated successfully.");
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-6">
                <div class="bg-gray-50 p-4 rounded border">
                    ...
                    ${!addr ? `
                        <p class="text-xs text-red-500 font-bold mb-2">No configuration address linked to profile.</p>
                    ...
                    <form id="paymentForm" class="space-y-3">
                        <label class="flex items-center space-x-3 border bg-white p-2.5 rounded cursor-pointer hover:bg-indigo-50">
                            ...
                            <div class="text-xs">
                                ...
                                <span class="text-gray-500">Instant routing protocol.</span>
                            </div>
                        </label>
                        <label class="flex items-center space-x-3 border bg-white p-2.5 rounded cursor-pointer hover:bg-indigo-50">
                            ...
                            <div class="text-xs">
                                ...
                                <span class="text-gray-500">Fully encrypted tokenized network handshake.</span>
                            </div>
                        </label>
                    </form>
                </div>
            ...
            <div class="bg-gray-50 p-4 rounded border h-fit flex flex-col justify-between space-y-4">
                ...
                ` : `
                    <button onclick="executePlaceOrder(${total})" class="w-full bg-yellow-400 hover:bg-yellow-500 text-indigo-950 font-black py-3 rounded-lg shadow uppercase text-xs tracking-wider transition">Authorize Remittance & Place Order</button>
                `}
            </div>
        </div>
    `;
...
function renderTracking() {
    ...
    div.innerHTML = `
        ...
        <div class="text-sm font-semibold text-gray-500 mb-6">Tracking Reference: <span class="text-indigo-900">${order.id}</span></div>
        ...
        <div class="flex items-center justify-between max-w-md mx-auto my-12 relative">
            ${order.status === 'Cancelled' ? `
                <div class="w-full bg-red-100 border border-red-400 p-4 rounded-lg text-red-700 font-bold text-sm">
                    <i class="fa fa-exclamation-triangle"></i> This delivery transaction sequence has been cancelled.
                </div>
...
window.saveModalProduct = function() {
    ...
    const obj = {
        ...
        sellerId: AppState.currentUser ? AppState.currentUser.id : 3
    };
...
window.deleteProductEntry = function(id) {
    if(confirm("Confirm deletion execution on item ID: " + id)) {
                    ...
                    ${db.orders.map(o => `
                        <div class="p-3 border rounded text-xs bg-gray-50 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                            <div>
                                ...
                                <div class="mt-1">Current Status Flag: <span class="font-bold uppercase text-yellow-700">${o.status}</span></div>
                            </div>
                            <div>
    ...
    div.innerHTML = `
        <div class="bg-emerald-950 text-white p-6 rounded-lg shadow-sm flex justify-between items-center flex-wrap gap-4">
            <div>
                ...
                <p class="text-xs text-emerald-200 mt-1">Merchant: ${AppState.currentUser.name}</p>
            </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm border">
            <h3 class="font-bold text-base text-indigo-950 mb-4 border-b pb-2"><i class="fa fa-boxes"></i> Merchant Tracking Hub</h3>
            <div class="overflow-x-auto text-xs">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-100 text-gray-600 uppercase text-[10px] font-black tracking-wider"><th class="p-2">Item ID Reference</th><th class="p-2">Display Title</th><th class="p-2">Price Allocation</th><th class="p-2">Stock Status</th><th class="p-2 text-center">Action Handlers</th></tr>
                    </thead>
                    <tbody>
                        ${sellerProds.map(p => `
                            <tr class="border-b hover:bg-gray-50">
